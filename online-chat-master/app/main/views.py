...
#     rname = request.args.get('rname', "")
#     b_user = r.keys('b_user-*')
#     b_user_list = [r.get(b).decode('utf-8') for b in b_user]
#     ulist = r.zrange("chat-" + rname, 0, -1)
#     messages = r.zrange("msg-" + rname, 0, -1, withscores=True)
#     msg_list = [[json.loads(i[0]), time.strftime("%Y/%m/%d %p%H:%M:%S", time.localtime(i[1]))] for i in messages]
#     if current_user.is_authenticated:
#         return render_template('chat.html', rname=rname, user_list=ulist, msg_list=msg_list, b_user_list=b_user_list)
#     else:
#         email = "youke" + "@hihichat.com"
#         hash = hashlib.md5(email.encode('utf-8')).hexdigest()
#         gravatar_url = 'http://www.gravatar.com/avatar/' + hash + '?s=40&d=identicon&r=g'
...
#         if b_user:
#             emit('receive_message', {'code': 201, 'msg': 'You are under block now!'}, broadcast=False)
#             return

#         if data['rtype'] == 'p':
#             r.zadd("pmsg-" + rname + '-' + current_user.username, {json.dumps({"username": current_user.username, "msg": message}): time.time()})
#             r.zadd("pmsg-" + current_user.username + '-' + rname, {json.dumps({"username": current_user.username, "msg": message}): time.time()})
#         else:
#             ulist = r.zrange("chat-" + rname, 0, -1)
#             if current_user.username.encode('utf-8') in ulist:
#                 body = {"username": current_user.username, "msg": message}
#                 r.zadd("msg-" + rname, {json.dumps(body): time.time()})
#             else:
#                 emit('receive_message', {'code': 403, 'msg': 'You are not in this room'}, broadcast=False)
#                 return

#         emit('receive_message', {'username': current_user.username, 'message': message}, broadcast=True)
#     else:
#         chat_text = get_bot_text(message)
#         emit('receive_message', {'username': 'bot', 'message': chat_text}, broadcast=True)
...
def pchat_info(user):
    ...
    for i in pchat:
        i_str = str(i)
        user1, user2 = i_str.split('-', 3)[1], i_str.split('-', 3)[2]
...
def send_chat(info):
    if current_user.is_authenticated:
        ...
        if rtype and rtype == 'p':
            # 将消息写入到当前用户与对方用户之间的键
            r.zadd("pmsg-" + rname + '-' + current_user.username, json.dumps({"username": current_user.username, "msg": info}), time.time())
            # 将消息写入到对方用户与当前用户之间的键
            r.zadd("pmsg-" + current_user.username + '-' + rname, json.dumps({"username": current_user.username, "msg": info}), time.time())
